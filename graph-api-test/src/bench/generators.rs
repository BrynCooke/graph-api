use crate::{Edge, Vertex, populate_graph, Refs};
    ...
    G: Graph<Vertex = Vertex, Edge = Edge>,
{
    ...
    for _ in 0..edge_count {
        ...
        let edge = match rng.gen_range(0..3) {
            0 => Edge::Knows { since: rng.gen_range(1980..2023) },
            1 => Edge::Created,
    ...
    G: Graph<Vertex = Vertex, Edge = Edge>,
{
    ...
    for (i, &project_id) in project_ids.iter().enumerate() {
        ...
        while deps.len() < num_deps {
            ...
            if target_idx != i { // Avoid self-dependencies
    ...
    for &project_id in &project_ids {
        ...
        for _ in 0..num_langs {
            ...
            graph.add_edge(
                project_id, 
                lang_id, 
                Edge::Language(crate::Language { name: "Rust".to_string() })
            );
        }
    }
...
fn random_biography(rng: &mut StdRng) -> String {
    let adjectives = ["creative", "diligent", "innovative", "experienced", "passionate"];
    let roles = ["developer", "engineer", "architect", "programmer", "coder"];
    let interests = ["graph databases", "distributed systems", "machine learning", "web development", "mobile apps"];
